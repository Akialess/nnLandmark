...
from nnlandmark.paths import nnLM_preprocessed
from nnlandmark.run.load_pretrained_weights import load_pretrained_weights
...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
    ...
    nnunet_trainer = recursive_find_python_class(join(nnlandmark.__path__[0], "training", "nnUNetTrainer"),
    ...
    if nnunet_trainer is None:
        raise RuntimeError(f'Could not find requested nnunet trainer {trainer_name} in '
                           f'nnlandmark.training.nnUNetTrainer ('
                           f'{join(nnlandmark.__path__[0], "training", "nnUNetTrainer")}). If it is located somewhere '
    ...
    preprocessed_dataset_folder_base = join(nnLM_preprocessed, maybe_convert_to_dataset_name(dataset_name_or_id))
...
def run_training_entry():
    ...
    parser.add_argument('-tr', type=str, required=False, default='nnLandmark',
                        help='[OPTIONAL] Use this flag to specify a custom trainer. Default: nnLandmark')
