...
import nnlandmark
from nnlandmark.paths import nnLM_preprocessed, nnLM_raw
from nnlandmark.preprocessing.cropping.cropping import crop_to_nonzero
...
class DefaultPreprocessor(object):
    ...
    def _normalize(self, data: np.ndarray, seg: np.ndarray, configuration_manager: ConfigurationManager,
        ...
        for c in range(data.shape[0]):
            ...
            normalizer_class = recursive_find_python_class(join(nnlandmark.__path__[0], "preprocessing", "normalization"),
                                                           scheme,
    ...
    def run(self, dataset_name_or_id: Union[int, str], configuration_name: str, plans_identifier: str,
        ...
        assert isdir(join(nnLM_raw, dataset_name)), "The requested dataset could not be found in nnLM_raw"

        plans_file = join(nnLM_preprocessed, dataset_name, plans_identifier + '.json')
        ...
        dataset_json_file = join(nnLM_preprocessed, dataset_name, 'dataset.json')
        dataset_json = load_json(dataset_json_file)

        output_directory = join(nnLM_preprocessed, dataset_name, configuration_manager.data_identifier)
        ...
        dataset = get_filenames_of_train_images_and_targets(join(nnLM_raw, dataset_name), dataset_json)
